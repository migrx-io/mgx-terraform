...
if grep -q mgx-core /etc/sudoers; then
        ...
        echo "%mgx-core     ALL=(ALL)       NOPASSWD:    /usr/sbin/mdadm, /usr/bin/mount, /usr/bin/umount, /usr/sbin/blkid, /usr/sbin/mkfs.ext4, /usr/sbin/nvme, /usr/bin/kill, /usr/bin/systemctl, /usr/sbin/arping, /usr/bin/apt, /usr/bin/apt-cache, /usr/sbin/ip" >> /etc/sudoers
