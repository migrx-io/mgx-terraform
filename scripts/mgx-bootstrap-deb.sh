...
if grep -q mgx-core /etc/sudoers; then
        ...
        echo "%mgx-core     ALL=(ALL)       NOPASSWD:    /opt/mgx-s3backer/scripts/mgx-s3backer, /usr/sbin/fstrim, /usr/sbin/mdadm, /usr/bin/mount, /usr/bin/umount, /usr/sbin/blkid, /usr/sbin/mkfs.ext4, /usr/sbin/mkfs.xfs, /usr/sbin/nvme, /usr/bin/kill, /usr/bin/systemctl, /usr/sbin/arping, /usr/bin/apt, /usr/bin/apt-cache, /usr/sbin/ip" >> /etc/sudoers
